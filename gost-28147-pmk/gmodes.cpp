...
// Encrypt in CNT mode (gamma)
void cnt_encrypt ( byte to[MAXT][BS], 
				   byte ts[MAXT][BS], 
				   byte iv[BS], 
				   u32bit n)
{	
	...
	for (j = 0; j < n; j++)
	{
		...
		for ( int i = 0; i < 4; i++ )
			tt1[i] = get_byte(3-i, N3); 
		for ( int i = 0; i < 4; i++ )
			tt1[i+4] = get_byte(3-i, N4);
...
// Decrypt data in CNT mode (gamma)
void cnt_decrypt( byte ts[MAXT][BS], 
				  byte td[MAXT][BS], 
				  byte iv[BS], 
				  u32bit n)
{
				   ...
				   u32bit n)
{
	...
	switch (mode)
	{
		...
		case CNT:		cnt_encrypt ( to, ts, iv, n);	break;
	...
	switch (mode)
	{
		...
		case CNT:		cnt_decrypt ( ts, td, iv, n);	break;
...
void run_test_block ( Mode mode, int oidnum )
{
	cout << "\nGOST 28147-89." << endl;
	cout << "Mode: ";	
	switch (mode)
	...
	// Output OID table
	// cout << OID[oidnum] << endl;
	output_oid( oidnum );

	for (int k = 0; k < 5; k++)
	{
		cout << "\nTEST " << k+1 << endl;
		run_one_test( mode, oidnum, to1, s1, kzu[k], 2);
	}

	cout << "\nTEST 6\n"; 
	run_one_test( mode, oidnum, to2, s2, kzu[4], 32);
}
